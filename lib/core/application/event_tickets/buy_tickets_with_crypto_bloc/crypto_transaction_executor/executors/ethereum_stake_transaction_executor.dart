...
class EthereumStakeTransactionExecutor implements CryptoTransactionExecutor {
  @override
  Future<CryptoTransactionExecutionResult> execute({
    ...
    try {
      if (erc20currencyAddress != zeroAddress) {
        erc20ApproveTxHash = await Web3Utils.approveSpender(
          ...
          from: from,
          spender: chain.stakePaymentContract ?? '',
          amount: amount,
          chain: chain,
        );
      }
    ...
    final contractCallTxn = Transaction.callContract(
      ...
      parameters: [
        BigInt.parse('0x${paymentAccount.accountInfo?.configId ?? ''}'),
        eventId,
        payment.id,
