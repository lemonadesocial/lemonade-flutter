...
class ConnectWalletProfileButton extends StatelessWidget {
  final User user;
  const ConnectWalletProfileButton({
    super.key,
    required this.user,
  });

  bool isMe(AuthSession? session) {
    ...
    return BlocBuilder<AuthBloc, AuthState>(builder: (context, authState) {
      return authState.maybeWhen(
          authenticated: (authSession) {
            if (!isMe(authSession)) return SizedBox.shrink();
            return _Button();
          },
